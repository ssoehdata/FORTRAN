...
!   to a straight line, and print ou thte resulting slope 
!   and intercept values. The input data for thsi fit 
!   comes from a user-specified input data file. 
    ...
    CHARACTER(len=24) :: filename      ! input filename (<= 24 chars)
    ...
    1000 FORMAT ('This program performs  a least-squares fit of an ',/, &
                'input data set ot a straight line. Enter the name' ,/ & 
    ...
    x_bar = sum_x / real(n) 
    y_bar = sum_y / real(n) 
    slope = (sum_xy - sum_x * sum_y) / ( sum_x2 - sum_x * x_bar)
