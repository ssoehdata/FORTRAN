...
!
!       Program contains error-case for either only one number entered, or no numbers.
!       This results in a divide by zero condition and NaN or unpredictable behaviour.
...
DO
        ! Read in next value 
        WRITE (*,*) 'Enter number (enter a negative number to finish data entry): '
        READ  (*,*) x
        WRITE (*,*) 'The number entered is ', x 
